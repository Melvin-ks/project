...
void SM3_update(sm3_ctx * ctx, uint8_t *buf, int len)
{
	while (len--)
	{
...
std::string SM3_256(std::string buf)
{
	std::string hash_value;
	uint8_t *temp=(uint8_t*)hash_value.c_str();
	uint8_t hash[32];
	sm3_ctx ctx;
	SM3_init(&ctx);
	SM3_update(&ctx, temp, buf.length());
	SM3_final(&ctx, hash);
	char *out_buff;
	
	for(int i = 0; i < 32; i++)
    {
      sprintf((char*)out_buff + (i * 2), "%02x", hash[i]);
    }
    out_buff[64] = 0;
	hash_value=out_buff;
	return hash_value;
